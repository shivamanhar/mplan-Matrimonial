$(document).ready(function(){	
	...
	handleFiles = function (files,e){
		...
		if (!file.type.match(imageType)) {  
		  alert("File \""+file.name+"\" is not a valid image file, please upload image in jpg formate");
		  return false;	
		} 
		// check file size
		if (parseInt(file.size / 1024) > 2050) {  
		  alert("File \""+file.name+"\" is too big. please select image in 500x300 px only");
		  return false;	
		} 
